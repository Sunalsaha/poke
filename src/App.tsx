...
type ConnectionState = 'trying' | 'connected' | 'cannot' | null;
type PicoState = 'idle' | 'listening' | 'thinking' | 'preparing';

const App: React.FC = () => {
  const [connectionState, setConnectionState] = useState<ConnectionState>('trying');
  const [picoState, setPicoState] = useState<PicoState>('idle');

  // Connection state management with automatic transitions
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout>;

    if (connectionState === 'trying') {
      timer = setTimeout(() => {
        const rand = Math.random();
        if (rand > 0.3) {
          setConnectionState('connected');
        } else {
          setConnectionState('cannot');
        }
      }, 3000);
    } else if (connectionState === 'cannot') {
      timer = setTimeout(() => {
        setConnectionState('trying');
      }, 5000);
    } else if (connectionState === 'connected') {
      timer = setTimeout(() => {
        setConnectionState(null); // Switch to PicoLogo
      }, 2000);
    }
  ...
  const handleBackgroundError = (e: React.SyntheticEvent<HTMLImageElement>): void => {
    console.warn('Background image failed to load');
    (e.target as HTMLImageElement).style.display = 'none';
  };

  return (
    <div className="min-h-screen text-white font-mono overflow-hidden relative select-none bg-black">
      {/* Background Layer */}
      <div className="fixed inset-0 z-0">
        <div className="w-full h-full relative overflow-hidden">
          <img 
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex gap-4 sm:gap-6 p-4 sm:p-6">
          {/* Left Panel */}
          <div className="flex-1 min-w-0">
            <SystemInfo side="left" />
          </div>
